...
class ReverieOfCopperSulfateAnalyzer:
    def __init__(self, master):
        self.master = master
        self.master.title("硫酸铜的遐想 - 甘肃新高考(3+1+2)数据分析舱 (终极无损版)")
    ...
    def _build_help_tab(self):
        ...
        guide_text = (
            ...
            "   • 默认比例：A(15%)、B(35%)、C(35%)、D(13%)、E(2%)。支持在面板内修改。\n\n"
    ...
    def assign_score_logic(self, series):
        ...
        # 核心：不能用 fillna(0) 补齐，未考者必须保持 NaN，防止拉低均分
    ...
    def _execute_computation(self, dialog, df, tracks):
        ...
        try:
            ...
            for track in self.tracks:
                ...
                # 🚀 核心修复：强行把 0 替换为 NaN，彻底踢出均分和标准差池子！
                for c in t_cols:
                ...
                # 总分计算：sum(axis=1)会自动把NaN当做0加起来，这很合理（缺考科目为0分计入总分）
                ...
                for col in calc_cols:
                    # 排名时，NaN会自动被忽略，不会出现9999并列倒数第一的情况
            ...
            # 视觉上将缺考排名的 9999 清除，把 NaN 转为空白，实现完美显示
