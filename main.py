...
import tempfile # 🌟 新增：用于获取系统绝对合法临时目录
...
# ================= 配置与全局变量 =================
# 🌟 核心修复 1：将配置文件保存在用户专属主目录，防止打包后因权限问题无法保存 API 密钥
CONFIG_FILE = os.path.join(os.path.expanduser("~"), ".chemtts_config.json")
...
# 🌟 核心修复 2：将临时试听文件放在系统的绝对临时文件夹中，彻底解决打开文件失败 (0x94) 错误
temp_preview_file = os.path.join(tempfile.gettempdir(), "chemtts_preview_audio.mp3")
