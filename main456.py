...
# ==========================================
# 核心修改：切换为极度稳定的阿里云 DeepSeek 节点
# ==========================================
DEFAULT_DEEPSEEK_URL = "https://dashscope.aliyuncs.com/compatible-mode/v1"
...
class TTSApp:
    def __init__(self, root):
        self.root = root
        self.root.title("DeepSeek 智能语音合成助手 - 作者: 俞晋全")
    ...
    def create_ui(self):
        ...
        ttk.Label(frame_status, text="Author: 俞晋全", bootstyle="secondary").pack(side=RIGHT, padx=10)
        ...
        # 5. AI 润色区
        frame_ai = ttk.Labelframe(self.root, text="DeepSeek AI 智能处理 (阿里云高速节点)", padding=15, bootstyle="success")
    ...
    def run_deepseek_polish(self):
        ...
        if not api_key:
            api_key = simpledialog.askstring("阿里云 API Key", "请输入阿里云百炼平台的 API Key (sk-开头):", show="*")
    ...
    def _deepseek_thread(self, text, api_key):
        self.update_status("正在连接阿里云 DeepSeek-V3 节点润色文本...")
        try:
            client = OpenAI(api_key=api_key, base_url=DEFAULT_DEEPSEEK_URL)
            response = client.chat.completions.create(
                # 修改点：使用阿里云支持的模型名称 deepseek-v3
                model="deepseek-v3",
                messages=[
            ...
            self.root.after(0, lambda: messagebox.showinfo("完成", "DeepSeek 润色已完成！(通过阿里云节点)"))
        except Exception as e:
